		...
		<script>
			$(document).ready(function(){
				...
				$("#materias, #bimestres").focus(function(){
					prev_val_materias = $("#materias").val();
					prev_val_bim = $("#bimestres").val();
				}).change(function(){
					$("#loader").show();
				...
				$("#add").on("click",function(){
					if($("#materias").val()!="0"){
						$("#save").attr("data-unsaved",1);
						$("#sortable").append(get_li("Nueva cualidad",0,0));
					}
				});
				...
				$("#save").on("click",function(){
					if($("#materias").val()!="0" && !$("#save").prop('disabled')){
						var lis=new Array();
						...
						$.post(window.url.base_url+"home/ctrhome/save_cualidades",{data:lis, idMateria:$("#materias").val(), grado:$("#materias option:selected").attr("data-grado"), bim:$("#bimestres").val()},function(resp){
							resp=JSON.parse(resp);
							if(resp.success!==false){
				...
				$("#sortable").on("click",".del",function(){
					...
					if(parseInt(id_main)>0){
						$.post(window.url.base_url+"home/ctrhome/consult_cualidad_to_delete",{id:parseInt(id_main)},function(resp){
							resp=JSON.parse(resp);
							var s=false;
